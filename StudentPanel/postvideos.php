...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if($stmts = $conn->query("select * from admin_register_student where username = '".$_SESSION['studentusername']."'")){
            while($n = $stmts->fetch_array(MYSQLI_ASSOC)){
                $fullname = $n['fullname'];
            }
        }
        
        if($stmt = $conn->prepare("insert into student_video_gallery set username = ?,title=?,description=?,file=?,status=?,fullname=?")){
            $stmt->bind_param('ssssss',$username,$title,$description,$file,$status,$fullname);
            $stmt->execute();
			...
			<div class="row">
				...
				<div class="col-md-4">
					...
					<div class="form-group text-center">
						...
						<a href="viewvideos.php" title="Click Here To View News Uploaded By You" target="_blank">View Video Gallery</a>
					</div>
