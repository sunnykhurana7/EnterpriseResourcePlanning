...
//       exit;
        ...
        if($stmt = $conn->prepare("insert into  teacher_add_subject set username=?,name=?,subject=?,course=?,shift=?,semester=?,date=?")){
            $stmt->bind_param('sssssss',$username,$name,$subject,$course,$shift,$semester,$date);
            $stmt->execute();
            if($stmt->affected_rows==1){
                header('location:subject.php?insert'); 
            }
        }
			...
			<div class="row">
			   ...
			   <div class="col-md-4 col-xs-12 col-lg-4">
				 ...
				 <div class="form-group">
					...
					<input type="text" class="form-control" placeholder="Data Structure"  name="subject" id="subject">
						<span id="sub"></span>
				 </div>
				 ...
				 <div class="form-group">
					...
					<button type="submit" name="submit" class="btn btn-primary btn-sm text-center" onclick="validation()">Submit</button>
				 </div>
                 
                 <div class="form-group text-center">
                    <span><a href="viewSubject.php">View Subject That You Added</a></span>                    
                 </div>
