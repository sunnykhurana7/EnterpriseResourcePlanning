<?php
    // connectivity code come here
    include '../includes/conn.php';    
?>


<?php
    // insert code comes here
    ...
    $date = date('Y-d-m');
        ...
        if($stmt = $conn->prepare("insert into  admin_register_coordinators set username=?,fullname =?,emailaddress=?,password=?,mobilenumber=?,course=?,year=?,date=?")){
              $stmt->bind_param('ssssssss',$username,$fullname,$emailaddress,$password,$mobilenumber,$course,$year,$date);
              $stmt->execute();
              if($stmt->affected_rows==1){
                  header('location:registercoordianors.php?insert');
              }else
              {
                  header('location:location:registercoordianors.php?errors');
              }  
        }
    }
?>
		...
		<form method="post" enctype="multipart/form-data">
			<div class="row" style="margin-bottom:15px;">
				<div class="col-md-12 col-lg-12 col-xs-12 text-center">
					<span style="font-size:25px;font-weight:bold;color:blue;">REGISTER NEW COORDINATORS</span>
				</div>
			</div>
			...
			<div class="container text-center">
				<div class="row">
					...
					<div class="col-md-4 col-xs-12 col-lg-4">
						...
						<div class="form-group">
							<label for="mobile number" class="pull-left">Enter Mobile Number</label>
							<input type="text" class="form-control" placeholder ="9871727979" required name="mobilenumber" id ="mobilenumber">	
						</div>
						
						
						<div class="form-group">
							<label for="selectcourse">Select Course</label><br>
							<select name ="course" >
						...
						<div class="form-group">
							<label for="selectyear">Select Year</label><br>
							<select name ="year">
								
								<option value="firstyear">First Year </option>
								<option value="secondyear">Second Year</option>
								<option value="thirdyear">Second Year</option>
						...
						<div class="form-group text-center">
							<a href="viewcoordinators.html" target="_blank" style="text-decoration:none;background-color:black;color:white;padding:5px;">View Details Of Register Coordiantors</a>
						</div>
